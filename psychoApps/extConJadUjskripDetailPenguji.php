...
<div class="table-responsive" style="margin-bottom:30px;">
   <table class="table m-0 table-bordered table-sm small custom">
      ...
      <tbody class="text-muted" style="font-size:13px;">
         <?php				           	  
            $no=0;
            $q =  "SELECT * FROM jadwal_ujskrip WHERE (sekretaris_penguji='$id' OR ketua_penguji='$id' OR penguji_utama='$id') AND id_ujskrip='$data[id_ujskrip]' AND tgl_ujian='$data[tgl_ujian]' ORDER BY jam_mulai ASC";
         ...
         <tr>
            ...
            <td class="text-left"><?php echo $mydata['judul_skripsi']=preg_replace('/<p[^>]*>(.*)<\/p[^>]*>/i', '$1', $mydata['judul_skripsi']);?></td>
            <td class="text-left <?php if($d['sekretaris_penguji']==$id) { echo 'bg-secondary';} else { echo '';}?>"><?php echo $dp1['nama'];?></td>
            <td class="text-left <?php if($d['ketua_penguji']==$id) { echo 'bg-secondary';} else { echo '';}?>"><?php echo $dp2['nama'];?></td>
            <td class="text-left <?php if($d['penguji_utama']==$id) { echo 'bg-secondary';} else { echo '';}?> pr-1"><?php echo $dp3['nama'];?></td>
         </tr>
         <?php 
            }
            ?>
