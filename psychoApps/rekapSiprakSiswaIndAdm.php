    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
            ...
            <div class="row">
              ...
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  ...
                  <li class="breadcrumb-item small">Izin Praktikum Individu Testee Siswa</li>
                </ol>
              </div>
            </div>
          </div>
        ...
        <?php include 'pagination.php';    
          ...
          $reload = "rekapSiprakSiswaIndAdm.php?pagination=true&date=$tahun&keyword=$keyword";
          ...
          WHERE (jesur.nim LIKE '%$keyword%' OR jesur.tujuan_prak LIKE '%$keyword%' OR jesur.nama_testee LIKE '%$keyword%' OR jesur.waktu LIKE '%$keyword%' OR jesur.jam LIKE '%$keyword%' OR jesur.tgl_pengajuan LIKE '%$keyword%' OR jesur.thn_pengajuan LIKE '%$keyword%' OR jesur.tgl_dikeluarkan LIKE '%$keyword%' OR jesur.tembusan LIKE '%$keyword%' OR jesur.catatan LIKE '%$keyword%' OR osp.nm LIKE '%$keyword%' OR dm.nama LIKE '%$keyword%' OR dk.nm_kota LIKE '%$keyword%' OR jt.nama LIKE '%$keyword%' OR dmkp.nama LIKE '%$keyword%' OR dp_pejabat.nama LIKE '%$keyword%' OR osps.nm LIKE '%$keyword%' OR ab1.username LIKE '%$keyword%' OR ab1.nm_person LIKE '%$keyword%' OR ab2.username LIKE '%$keyword%' OR ab2.nm_person LIKE '%$keyword%') AND (jesur.jenis_praktikum='1') AND (jesur.thn_pengajuan='$tahun') GROUP BY jesur.id ORDER BY jesur.statusform ASC,ABS(jesur.tgl_pengajuan) DESC";
          
          $result = mysqli_query($con, $sql);
          }else{
          $reload = "rekapSiprakSiswaIndAdm.php?pagination=true";
          $sql = "SELECT * FROM siprak_siswa WHERE jenis_praktikum='1' AND thn_pengajuan='$tahun' ORDER BY statusform ASC,ABS(tgl_pengajuan) DESC";
          $result = mysqli_query($con, $sql);
          }
          ...
          <div class="container-fluid">
            <div class="row">
              <div class="col-sm mb-2">
                ...
                <form method="post" action="rekapSiprakSiswaIndAdm.php?date=<?php echo $tahun;?>&page=<?php echo $page;?>">
                  <div class="input-group">
                    ...
                    <div class="input-group-append">
                      ...
                      <?php
                        ...
                        '<a class="btn btn-sm btn-warning" title="Kembali" href="rekapSiprakSiswaIndAdm.php?date='.$tahun.'&page='.$page.'"><i class="fas fa-sync"></i> Kembali</a>';}
                        ?>
                    </div>
                  </div>
                </form>
                      ...
                      <table class="table table-hover table-sm custom mb-0">
                        ...
                        <tbody>
                          ...
                          <tr data-widget="expandable-table" aria-expanded="false">
                            ...
                            <?php if($data['statusform']=='3') { echo '<td class="text-center" width="4%"><a role="button" class="btn btn-block btn-outline-warning btn-xs" onclick="return confirm(\'Status surat telah selesai!\')" title="Status surat telah selesai!" disabled><i class="bi bi-pencil-square"></i></a></td>';} else { echo '<td class="text-center" width="4%"><a href="editSiprakisAdm.php?id='.$data['id'].'&page='.$page.'" role="button" class="btn btn-block btn-outline-warning btn-xs" title="Proses surat!"><i class="bi bi-pencil-square"></i></a></td>';}?>
                            <?php if($data['statusform']=='1' OR $data['statusform']=='4') { echo '<td class="text-center" width="4%"><a role="button" class="btn btn-block btn-outline-secondary btn-xs" onclick="return confirm(\'Status surat masih pengajuan atau ditolak!\')" title="Status surat masih pengajuan atau ditolak!" disabled><i class="bi bi-printer"></i></a></td>';} else { echo '<td class="text-center" width="4%"><a href="cetakSiprakisAdm.php?id='.$data['id'].'&page='.$page.'" role="button" class="btn btn-block btn-outline-secondary btn-xs" target="_blank" title="Cetak surat!"><i class="bi bi-printer"></i></a></td>';}?>
                            <?php if($data['statusform']=='2' OR $data['statusform']=='3') { echo '<td class="text-center" width="4%"><a role="button" class="btn btn-block btn-outline-danger btn-xs" onclick="return confirm(\'Status surat masih proses atau telah selesai!\')" title="Status surat masih proses atau telah selesai!" disabled><i class="bi bi-trash"></i></a></td>';} else { echo '<td class="text-center" width="4%"><a href="deleteSiprakisAdm.php?id='.$data['id'].'&date='.$tahun.'&page='.$page.'" role="button" class="btn btn-block btn-outline-danger btn-xs" onClick="return confirm(\'Yakin data ini dihapus?\')" title="Hapus surat!"><i class="bi bi-trash"></i></a></td>';}?>
                            <td width="10%" class="text-center"><?php if($data['statusform']=='1' OR $data['statusform']=='4') {  echo '<a href="catatanSiprakisAdm.php?id='.$data['id'].'&date='.$tahun.'&page='.$page.'" type="button" class="btn btn-outline-success btn-xs btn-block" title="Beri catatan"><i class="far fa-comment-alt"></i> Beri Catatan</a>';} else { echo '<a href="#" type="button" class="btn btn-outline-secondary btn-xs btn-block" onclick="return confirm(\'Catatan hanya diberikan jika status surat ditolak!\')" title="Catatan hanya diberikan jika status surat ditolak!"><i class="far fa-comment-alt"></i> Beri Catatan</a>';}?></td>
                            <td class="text-center p-2">
                              <form action="updateStatusformSiprakisAdm.php" method="post" enctype="multipart/form-data">
