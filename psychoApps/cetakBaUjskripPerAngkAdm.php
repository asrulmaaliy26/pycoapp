      ...
      <div class="content-wrapper">
        <div class="content-header">
          <div class="container-fluid">
            <div class="row">
              <div class="col-sm-6">
                <h6 class="m-0">Form Berita Acara Ujian Skripsi Angkatan <?php echo $angkatan;?></h6>
              </div>
              <div class="col-sm-6">
                    ...
                    <div class="table-responsive">
                      <table class="table table-hover m-0 table-bordered text-center table-sm small custom">
                        ...
                        <tbody>
                          <?php
                            $no=0;
                            $sql = "SELECT * FROM peserta_ujskrip INNER JOIN dt_mhssw ON peserta_ujskrip.nim=dt_mhssw.nim WHERE peserta_ujskrip.angkatan='$angkatan' ORDER BY dt_mhssw.nama ASC";
                          ...
                          <tr>
                            ...
                            <td width="6%" class="text-center"> <?php if(empty($dt_jdwl['tgl_ujian']) || empty($dt_jdwl['jam_mulai']) || empty($dt_jdwl['jam_selesai']) || empty($dt_jdwl['ruang'])) { echo '<a type="button" class="btn btn-outline-secondary btn-flat btn-xs btn-block" title="Tidak ada data" onclick="return confirm(\'Tidak ada data\')"><i class="fas fa-print"></i></a>';} else { echo '<a href="cetakBaUjskripKosongPerAngkAdm.php?id='.$data['id'].'&page='.$page.'" type="button" class="btn btn-outline-warning btn-flat btn-xs btn-block" title="Cetak berita acara kosong" target="_blank"><i class="fas fa-print"></i></a>';} ?> </td>
                            <td width="6%" class="text-center pr-1"> <?php if($dt_nilai['nilai_sekretaris']==0 || $dt_nilai['nilai_ketua']==0 || $dt_nilai['nilai_utama']==0 || $dt_nilai['mean_nilai']==0) { echo '<a type="button" class="btn btn-outline-secondary btn-flat btn-xs btn-block" title="Tidak ada data" onclick="return confirm(\'Tidak ada data\')"><i class="fas fa-print"></i></a>';} else { echo '<a href="cetakBaUjskripIsiPerAngkAdm.php?id='.$data['id'].'&page='.$page.'" type="button" class="btn btn-outline-success btn-flat btn-xs btn-block" title="Cetak berita acara terisi"><i class="fas fa-print"></i></a>';} ?> </td>
                          </tr>
                          <?php
                            }
                            ?>
