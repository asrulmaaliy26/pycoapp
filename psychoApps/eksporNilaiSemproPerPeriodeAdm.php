...
<html lang="en">
  <head>
    <title>Data Nilai Seminar Proposal <?php echo 'Tahap '.$dthp['tahap'].' '.$dsemester['nama'].' '.$dnta['ta'].'';?></title>
    <meta charset="utf-8">
    <style>
      table {
  ...
  <body>
    <?php
      ...
      header('Content-Disposition: attachment; filename=Data Nilai Seminar Proposal Tahap '.$dthp['tahap'].' '.$dsemester['nama'].' '.$dnta['ta'].'.xls');
      ?>
    <table style="border:none;">
      <thead>
        <tr style="border:none;">
          <th style="border:none;" colspan="12">Data Nilai Seminar Proposal <?php echo 'Tahap '.$dthp['tahap'].' '.$dsemester['nama'].' '.$dnta['ta'].'';?>
          </th>
        </tr>
        <tr style="border:none;">
    ...
    <table>
      <thead>
        ...
        <tr style="text-align:center;">
          <td width="8%">Penguji I</td>
          <td width="8%">Penguji II</td>
          <td width="6%">Tanggal</td>
          <td width="6%">Pukul</td>
          <td width="6%">Ruang</td>
          <td width="6%">Penguji I</td>
          <td width="6%">Catatan</td>
          <td width="6%">Penguji II</td>
          <td width="6%">Catatan</td>
          <td width="6%">Rerata</td>
        </tr>
      </thead>
      <tbody>
        <?php
          $no=0;
          $sql = "SELECT * FROM peserta_sempro INNER JOIN dt_mhssw ON peserta_sempro.nim=dt_mhssw.nim INNER JOIN nilai_sempro ON peserta_sempro.id=nilai_sempro.id_pendaftaran INNER JOIN jadwal_sempro ON peserta_sempro.id=jadwal_sempro.id_pendaftaran WHERE peserta_sempro.id_sempro='$id' ORDER BY dt_mhssw.nama ASC";
        ...
        <tr>
          ...
          <td style="text-align:center;"> <?php include("nilaiNarsum1PesSemproAdm.php");?> </td>
          <td style="text-align:center;"> <?php if(empty($data['catatan_narsum1'])) { echo "Tidak ada";} else { echo nl2br($data['catatan_narsum1']);}?> </td>
          <td style="text-align:center;"> <?php include("nilaiNarsum2PesSemproAdm.php");?> </td>
          <td style="text-align:center;"> <?php if(empty($data['catatan_narsum2'])) { echo "Tidak ada";} else { echo nl2br($data['catatan_narsum2']);}?> </td>
