            ...
            <div class="row">
              ...
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  ...
                  <li class="breadcrumb-item active small">Judul Skripsi Telah Diverifikasi</li>
                </ol>
              </div>
            </div>
          </div>
              ...
              <section class="col-md-12 connectedSortable">
                <div class="card card-outline card-info">
                  <div class="card-header">
                    <div class="clearfix">
                      <h4 class="card-title float-left">Judul Skripsi Telah Diverifikasi</h4>
                    </div>
                  </div>
                      ...
                      <table class="table table-hover m-0 table-bordered text-center table-sm small custom">
                        <thead>
                          <tr class="text-center bg-secondary">
                            ...
                            <td width="30%">Nama</td>
                            <td width="46%">Judul Skripsi</td>
                            <td width="10%">Tgl. Pengajuan</td>
                            <td width="10%" class="pr-1">Tgl. Verifikasi</td>
                          </tr>
                        </thead>
                        <tbody>
                          <?php
                            $no=0;
                            $sql = "SELECT * FROM pengelompokan_dospem_skripsi WHERE id_periode='$id' AND cekjudul != '1' ORDER BY NIM ASC";
                          ...
                          <tr data-widget="expandable-table" aria-expanded="false">
                            ...
                            <td class="text-left"> <?php if($data['cekjudul']==1) { echo $data['judul_skripsi']=preg_replace('/<p[^>]*>(.*)<\/p[^>]*>/i', '$1', $data['judul_skripsi']).' <i class="fas fa-check text-warning" title="'.$dcekjudul['nm'].'"></i>';} else if($data['cekjudul']==2) { echo $data['judul_skripsi']=preg_replace('/<p[^>]*>(.*)<\/p[^>]*>/i', '$1', $data['judul_skripsi']).' <i class="fas fa-check text-success" title="'.$dcekjudul['nm'].'"></i>';} else if($data['cekjudul']==3) { echo $data['judul_skripsi']=preg_replace('/<p[^>]*>(.*)<\/p[^>]*>/i', '$1', $data['judul_skripsi']).' <i class="fas fa-check text-danger" title="'.$dcekjudul['nm'].'"></i>';} else if($data['cekjudul']==4) { echo $data['judul_skripsi']=preg_replace('/<p[^>]*>(.*)<\/p[^>]*>/i', '$1', $data['judul_skripsi']).' <i class="fas fa-times text-dark" title="'.$dcekjudul['nm'].'"></i>';}?> </td>
                            <td class="text-center"> <?php echo $data['tgl_pengajuan'];?> </td>
                            <td class="text-center pr-1"> <?php echo $data['tgl_cekjudul'];?> </td>
                          </tr>
