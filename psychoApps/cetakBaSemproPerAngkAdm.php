<?php include( "contentsConAdm.php" );
  ...
  $page = mysqli_real_escape_string($con,  $_GET[ 'page0' ] );
  ?>
<!DOCTYPE html>
<html lang="en">
      ...
      <div class="content-wrapper">
        <div class="content-header">
          <div class="container-fluid">
            <div class="row">
              <div class="col-sm-6">
                <h6 class="m-0">Form Berita Acara Seminar Proposal Angkatan <?php echo $angkatan;?></h6>
              </div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  <li class="breadcrumb-item small"><a class="text-info" href="rekapBaSemproAdm.php?page=<?php echo $page;?>">Form Berita Acara Seminar Proposal</a></li>
                    ...
                    <div class="table-responsive">
                      <table class="table table-hover m-0 table-bordered text-center table-sm small custom">
                        ...
                        <tbody>
                          <?php
                            $no=0;
                            $sql = "SELECT * FROM peserta_sempro INNER JOIN dt_mhssw ON peserta_sempro.nim=dt_mhssw.nim WHERE peserta_sempro.angkatan='$angkatan' ORDER BY dt_mhssw.nama ASC";
                          ...
                          <tr>
                            ...
                            <td class="text-center"> <?php echo $dt_jdwl['tgl_seminar'];?> </td>
                            <td class="text-center"> <?php echo $dt_jdwl['jam_mulai'].'-'.$dt_jdwl['jam_selesai'];?> </td>
                            <td class="text-center"> <?php echo $dt_ruang['nm'];?> </td>
                            <td width="6%" class="text-center"> <?php if(empty($dt_jdwl['tgl_seminar']) || empty($dt_jdwl['jam_mulai']) || empty($dt_jdwl['jam_selesai']) || empty($dt_jdwl['ruang'])) { echo '<a type="button" class="btn btn-outline-secondary btn-flat btn-xs btn-block" title="Tidak ada data" onclick="return confirm(\'Tidak ada data\')"><i class="fas fa-print"></i></a>';} else { echo '<a href="cetakBaSemproKosongPerAngkAdm.php?id='.$data['id'].'&page='.$page.'" type="button" class="btn btn-outline-warning btn-flat btn-xs btn-block" title="Cetak berita acara kosong" target="_blank"><i class="fas fa-print"></i></a>';} ?> </td>
                            <td width="6%" class="text-center pr-1"> <?php if($dt_nilai['nilai_narsum1']==0 || $dt_nilai['nilai_narsum2']==0 || $dt_nilai['mean_nilai']==0) { echo '<a type="button" class="btn btn-outline-secondary btn-flat btn-xs btn-block" title="Tidak ada data" onclick="return confirm(\'Tidak ada data\')"><i class="fas fa-print"></i></a>';} else { echo '<a href="cetakBaSemproIsiPerAngkAdm.php?id='.$data['id'].'&page='.$page.'" type="button" class="btn btn-outline-success btn-flat btn-xs btn-block" title="Cetak berita acara terisi"><i class="fas fa-print"></i></a>';} ?> </td>
                          </tr>
                          <?php
                            }
                            ?>
