...
$qNim = "SELECT nim FROM pengelompokan_dospem_skripsi WHERE dospem_skripsi1='$dtDosen[id]' LIMIT 1";
...
$dJml = mysqli_fetch_assoc($rJml);
$jData = $dJml['jumData'] ?? 0;

$qJmlProses = "SELECT COUNT(*) AS jumData FROM pengelompokan_dospem_skripsi WHERE dospem_skripsi1='$dtDosen[id]' AND nim IN(SELECT nim FROM peserta_ujskrip)";
$rJmlProses =  mysqli_query($con, $qJmlProses) or die(mysqli_error($con));
$dJmlProses = mysqli_fetch_assoc($rJmlProses);
$jDataProses = $dJmlProses['jumData'] ?? 0;

$qJmlUjskrip = "SELECT COUNT(*) AS jumData FROM pengelompokan_dospem_skripsi WHERE dospem_skripsi1='$dtDosen[id]' AND nim NOT IN(SELECT nim FROM peserta_ujskrip)";
$rJmlUjskrip =  mysqli_query($con, $qJmlUjskrip) or die(mysqli_error($con));
$dJmlUjskrip = mysqli_fetch_assoc($rJmlUjskrip);
$jDataUjskrip = $dJmlUjskrip['jumData'] ?? 0;
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      ...
      if (isset($_REQUEST['keyword']) && $_REQUEST['keyword'] <> "") {
        ...
        $reload = "pembimbingSkripsi1.php?pagination=true&keyword=$keyword";
        ...
        $sql = "SELECT pds.id
          ...
          WHERE (pds.nim LIKE '%$keyword%' OR dm.nim LIKE '%$keyword%' OR ps.nim LIKE '%$keyword%' OR dm.nama LIKE '%$keyword%' OR dospem_skripsi1.nama LIKE '%$keyword%') AND (pds.dospem_skripsi1=$dtDosen[id]) ORDER BY dm.nama ASC";

        $result = mysqli_query($con, $sql);
      } else {
        $reload = "pembimbingSkripsi1.php?pagination=true";
        $sql = "SELECT * FROM pengelompokan_dospem_skripsi WHERE dospem_skripsi1='$dtDosen[id]' ORDER BY id DESC";
        $result = mysqli_query($con, $sql);
      }
        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col-sm mb-2">
              <form method="post" action="pembimbingSkripsi1.php">
                ...
                <div class="input-group">
                  ...
                  <div class="input-group-append">
                    ...
                    if (isset($_REQUEST['keyword']) && $_REQUEST['keyword'] <> "") {
                    ?>
                      <a class="btn btn-sm btn-warning" title="Kembali" href="pembimbingSkripsi1.php"><i class="fas fa-sync"></i> Kembali</a>
                    <?php
                    }
                    ?>
                  </div>
                ...
                <div class="card-header">
                  ...
                  <div class="card-tools">
                    <ul class="nav nav-pills ml-auto">
                      ...
                      <li class="nav-item">
                        <a class="nav-link" href="pembimbingSkripsi1Selesai.php">Selesai (<?php echo $jDataUjskrip; ?>)</a>
                      </li>
                    </ul>
                  </div>
                </div>
                    ...
                    <table class="table table-hover table-bordered table-sm custom mb-0">
                      ...
                      <tbody>
                        ...
                        while (($count < $rpp) && ($i < $tcount)) {
                          ...
                          if ($dnilai['nilai_narsum1'] == '0' && $dnilai['nilai_narsum2'] != '0') {
                            $nilaiakhir = $dnilai['nilai_narsum2'] / 1;
                          } elseif ($dnilai['nilai_narsum1'] != '0' && $dnilai['nilai_narsum2'] == '0') {
                            $nilaiakhir = $dnilai['nilai_narsum1'] / 1;
                          } elseif ($dnilai['nilai_narsum1'] == '0' && $dnilai['nilai_narsum2'] == '0') {
                            $nilaiakhir = ($dnilai['nilai_narsum1'] + $dnilai['nilai_narsum2']) / 2;
                          } elseif ($dnilai['nilai_narsum1'] != '0' && $dnilai['nilai_narsum2'] != '0') {
                            $nilaiakhir = ($dnilai['nilai_narsum1'] + $dnilai['nilai_narsum2']) / 2;
                          }
                        ?>
                                  ...
                                  <table class="table no-border rounded-0 mb-0">
                                    <tbody>
                                      ...
                                      <tr>
                                        ...
                                        <td>:</td>
                                        <td><?php if (empty($djdwl['tgl_seminar']) && empty($djdwl['jam_mulai']) && empty($djdwl['jam_selesai']) && empty($djdwl['ruang'])) {
                                              echo "Belum terjadwal";
                                            } else {
                                              echo $dayList[$day] . ', ' . bulanIndo($djdwl['tgl_seminar']) . ', ' . $djdwl['jam_mulai'] . ' - ' . $djdwl['jam_selesai'] . ' WIB.';
                                            } ?></td>
                                      </tr>
                                      <tr>
