<?php
  ...
  $myqry1="UPDATE pengelompokan_dospem_skripsi SET cek1='$cek1',tgl_cek1='$tgl_cek1' WHERE id='$id' LIMIT 1";
  mysqli_query($con, $myqry1) or die(mysqli_error($con));
  
  $qcekjudul = "SELECT cek1 FROM pengelompokan_dospem_skripsi WHERE id='$id'";
  $rcekjudul = mysqli_query($con,  $qcekjudul )or die( mysqli_error($con) );
  $dcekjudul = mysqli_fetch_assoc( $rcekjudul );
  
  if($dcekjudul['cek1'] == '1' || $dcekjudul['cek1'] == '4') {
  $myqry2="UPDATE pengelompokan_dospem_skripsi SET tgl_cek1='', tgl_mulai='',thn_mulai='',status='1' WHERE id='$id' LIMIT 1";
