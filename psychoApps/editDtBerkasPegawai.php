<?php include( "contentsConAdm.php" );
  ...
  $q_bp = "SELECT * FROM berkas_pegawai WHERE id='$id'";
  ...
  $q = "SELECT * FROM dt_pegawai WHERE id='$nip'";
  ...
  if(isset($_POST['new']) && $_POST['new']==1)
  {
    ...
    { 
      ...
      $newname = ''.$nm_berkas.'' .'_' . $id_image .'_'.uniqid().'.' . end($temp);
      $berkas = $foldername . $newname;
      move_uploaded_file($_FILES['berkas']['tmp_name'], $foldername . '/' . $newname);
  
      $res = mysqli_query($con, "SELECT berkas FROM berkas_pegawai WHERE id='$id' LIMIT 1");
      $d=mysqli_fetch_assoc($res);
      if (strlen($d['berkas'])>3)
      {
        if (file_exists($d['berkas'])) unlink($d['berkas']);
      }
      mysqli_query($con, "UPDATE berkas_pegawai SET berkas='$berkas' WHERE id='$id' LIMIT 1");
      header("location:dtBerkasPerPegawai.php?id=$id_pegawai&page=$page&message=notifEdit");
    } else 
    {
      header("location:dtBerkasPerPegawai.php?id=$id_pegawai&page=$page&message=notifGagalImage");
    }
  }
  mysqli_close($con);
  ?>
            ...
            <div class="row mb-2">
              ...
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  ...
                  <li class="breadcrumb-item small"><a class="text-info" href="dtBerkasPerPegawai.php?id=<?php echo $nip;?>&page=<?php echo $page;?>"><?php echo $data['nama_tg'];?></a></li>
                  <li class="breadcrumb-item active small">Edit Berkas Pegawai</li>
                </ol>
              </div>
            </div>
          </div>
          ...
          <div class="container-fluid">
            <div class="row">
              <section class="col-md-12 connectedSortable">
                <form action="editDtBerkasPegawai.php?id=<?php echo $id;?>&page=<?php echo $page;?>" method="post" enctype="multipart/form-data">
                  <div class="card card-outline card-success">
                    <div class="card-header">
                      <div class="clearfix">
                        <h4 class="card-title float-left">Edit Berkas Pegawai [<?php echo '<span class="text-muted small">'.$data_kbp['nm'].'</span>';?>]</h4>
                        <span class="float-right"><strong><?php echo $data['nama_tg'];?></strong></span>
                      </div>
                    </div>
                    <div class="card-body">
                      <input type="text" name="id" class="sr-only" value="<?php echo $data_bp['id'];?>" required readonly>
                    ...
                    <div class="card-footer justify-content-between">
                      ...
                      <a href="dtBerkasPerPegawai.php?id=<?php echo $nip;?>&page=<?php echo $page;?>" class="btn btn-outline-secondary btn-sm">Batal</a>
                    </div>
                  </div>
                </form>
              </section>
