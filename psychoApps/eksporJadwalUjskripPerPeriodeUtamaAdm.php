<?php include( "contentsConAdm.php" );
  ...
  $qidjdwl = "SELECT * FROM jadwal_ujskrip WHERE penguji_utama='$id' AND id_ujskrip='$id_ujskrip'";
  $ridjdwl = mysqli_query($con, $qidjdwl)or die( mysqli_error($con));
  $didjdwl = mysqli_fetch_assoc($ridjdwl);

  $qpenguji = "SELECT * FROM dt_pegawai WHERE id='$didjdwl[penguji_utama]'";
...
<html lang="en">
  ...
  <body>
    <?php
      ...
      $jdwl = "SELECT * FROM jadwal_ujskrip WHERE penguji_utama='$id' AND id_ujskrip='$id_ujskrip' AND tgl_ujian='$tgl_ujian'";
      $rjdwl = mysqli_query($con,  $jdwl )or die( mysqli_error($con) );
      $djdwl = mysqli_fetch_assoc( $rjdwl );
         
      $qpenguji = "SELECT * FROM dt_pegawai WHERE id='$djdwl[penguji_utama]'";
      $rqpenguji = mysqli_query($con, $qpenguji)or die( mysqli_error($con));
      $dqpenguji = mysqli_fetch_assoc($rqpenguji);
      ...
      include("extEksporJadwalUjskrip1Utama.php");
      include("extEksporJadwalUjskrip2Utama.php");
      include("extEksporJadwalUjskrip3Utama.php");?>
