    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <?php include 'pagination.php';
          $reload = "dataKontribEksAskPertahunAdm.php?pagination=true";
          $sql = "SELECT * FROM surat_keluar WHERE tahun = '$tahun' AND executor = '$username' GROUP BY bulan ORDER BY bulan ASC";
                      ...
                      <table class="table table-hover table-sm custom mb-0">
                        ...
                        <tbody>
                          <?php
                            ...
                            $qry = "SELECT COUNT(*) AS jumData FROM surat_keluar WHERE executor = '$username' AND tahun = '$data[tahun]' AND bulan = '$data[bulan]'";
                            $r =  mysqli_query($con, $qry) or die(mysqli_error($con));
                            $djum = mysqli_fetch_assoc($r) or die(mysqli_error($con));
                            ?>
                          <tr>
                            <td class="text-center pl-1"><?php echo ++$no_urut;?></td>
                            <td class="text-center"><?php echo $month[(int)$data['bulan']];?></td>
                            <td class="text-center"><?php echo $djum['jumData'];?></td>
                            <td class="text-center pr-2">
                              <a href="dataKontribEksAskPerbulanAdm.php?page=<?php echo $page;?>&tahun=<?php echo $tahun;?>&bulan=<?php echo $data['bulan'];?>" role="button" class="btn btn-warning btn-block btn-xs" title="Buka"><i class="bi bi-folder2-open"></i></a>
                            </td>
                          </tr>
                          <?php
                            $i++; 
