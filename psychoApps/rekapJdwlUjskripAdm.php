      ...
      <div class="content-wrapper">
        <div class="content-header">
          <div class="container-fluid">
            <div class="row">
              <div class="col-sm-6">
                <h6 class="m-0">Jadwal Ujian Skripsi</h6>
              </div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  <li class="breadcrumb-item active small">Jadwal Ujian Skripsi</li>
                </ol>
              </div>
            </div>
          </div>
        ...
        <?php
          include 'pagination.php';
           $reload = "rekapJdwlUjskripAdm.php?pagination=true";
              ...
              <section class="col-md-12 connectedSortable">
                <div class="card card-outline card-info">
                  <div class="card-header">
                    <div class="clearfix">
                      <h4 class="card-title float-left">Jadwal Ujian Skripsi</h4>
                    </div>
                  </div>
                      ...
                      <table class="table table-hover m-0 table-bordered text-center table-sm small custom">
                        ...
                        <tbody>
                          <?php
                            ...
                            $qry2 = "SELECT COUNT(id) AS jumData FROM peserta_ujskrip WHERE id_ujskrip='$id_ujskrip' AND statusform='2'";
                            $has2 = mysqli_query($con,  $qry2 )or DIE( mysqli_error($con) );
                            $data2 = mysqli_fetch_assoc( $has2 );
  
                            $qry3 = "SELECT COUNT(id) AS jumData FROM peserta_ujskrip WHERE id_ujskrip = '$id_ujskrip'";
                          ...
                          <tr>
                            ...
                            <td class="text-center"> <?php if($data1['jumData']==0) { echo '<a type="button" class="btn btn-outline-secondary btn-flat btn-xs btn-block" onclick="return confirm(\'Tidak ada data\')" title="Tidak ada data">'.$data1['jumData'].'</a>';} else { echo '<a href="jadUjskripPendingAdm.php?id='.$id_ujskrip.'&page='.$page.'" type="button" class="btn btn-outline-primary btn-flat btn-xs btn-block" title="Lihat data">'.$data1['jumData'].'</a>';} ?> </td>
                            <td class="text-center"> <?php if($data2['jumData']==0) { echo '<a type="button" class="btn btn-outline-secondary btn-flat btn-xs btn-block" onclick="return confirm(\'Tidak ada data\')" title="Tidak ada data">'.$data2['jumData'].'</a>';} else { echo '<a href="jadUjskripSelesaiAdm.php?id='.$id_ujskrip.'&page='.$page.'" type="button" class="btn btn-outline-primary btn-flat btn-xs btn-block" title="Lihat data">'.$data2['jumData'].'</a>';} ?> </td>
                            <td class="text-center"> <?php if($data3['jumData']==0) { echo '<a type="button" class="btn btn-outline-secondary btn-flat btn-xs btn-block" onclick="return confirm(\'Tidak ada data\')" title="Tidak ada data">'.$data3['jumData'].'</a>';} else { echo '<a href="rekapPesUjskripPerPeriodeJadAdm.php?id='.$id_ujskrip.'&page='.$page.'" type="button" class="btn btn-outline-primary btn-flat btn-xs btn-block" title="Lihat data">'.$data3['jumData'].'</a>';} ?> </td>
                            <td class="text-center pr-1"> <?php if($data2['jumData']==0) { echo '<a type="button" class="btn btn-outline-secondary btn-flat btn-xs btn-block" title="Tidak ada data" onclick="return confirm(\'Tidak ada data\')"><i class="fas fa-print"></i></a>';} else { echo '<a href="jadUjskripPerPeriodeAdm.php?id='.$id_ujskrip.'&page='.$page.'" type="button" class="btn btn-outline-warning btn-flat btn-xs btn-block" title="Cetak jadwal"><i class="fas fa-print"></i></a>';} ?> </td>
                          </tr>
                          <?php
                            $i++; 
                            $count++;
