  ...
  <body class="hold-transition sidebar-mini layout-fixed">
                ...
                <div class="card card-success card-outline card-outline-tabs">
                  <div class="card-header p-0 border-bottom-0">
                    <ul class="nav nav-tabs" role="tablist">
                      ...
                      <li class="nav-item">
                        <a class="nav-link active" href="riwayatPendaftaranUjianKompreUser.php" role="tab" aria-selected="true">Riwayat</a>
                      </li>
                    </ul>
                  </div>
                    ...
                    <div class="table-responsive">
                      <table class="table table-hover m-0 text-center table-sm custom">
                        ...
                        <tbody>
                          <?php
                            $no=0;
                            $sql = "SELECT * FROM peserta_kompre WHERE nim='$username'";
                            ...
                            $qry_jdwl = "SELECT * FROM jadwal_kompre WHERE id_kompre='$data[id_kompre]'";
                            $res_jdwl = mysqli_query($con, $qry_jdwl);
                            $dt_jdwl = mysqli_fetch_assoc($res_jdwl);
                            $oldDateJad = $dt_jdwl['tgl_kompre'];
                            $newDateJad = date("d-m-Y", strtotime($oldDateJad));
                            
                            $qry_p1 = "SELECT * FROM dt_pengawas_kompre WHERE id='$dt_jdwl[pengawas1]'";
                            $res_p1 = mysqli_query($con, $qry_p1);
                            $dt_p1 = mysqli_fetch_assoc($res_p1);
                            
                            $qry_p2 = "SELECT * FROM dt_pengawas_kompre WHERE id='$dt_jdwl[pengawas2]'";
                            ...
                            $qry_nilai = "SELECT * FROM nilai_kompre WHERE id_pendaftaran='$data[id]'";
                          ...
                          <tr data-widget="expandable-table" aria-expanded="false">
                            ...
                            <td width="4%" class="text-center pr-1"><a class='btn btn-outline-primary btn-xs btn-block' title='Jadwal seminar proposal skripsi' href='jadwalKompreUser.php?id=<?php echo $data['id_kompre'];?>'><i class="far fa-calendar-alt"></i></a></td>
                          </tr>
                                    ...
                                    <div class="card">
                                      ...
                                      <div class="card-body text-left pb-0">
                                        <dl class="row">
                                          ...
                                          <dd class="col-sm-8"><?php echo 'Tahap '.$dthp['tahap'].' '.$djp['nm'].' '.$dsemester['nama'].' '.$dnta['ta'].'';?></dd>
                                          <dt class="col-sm-4">SKS yang telah ditempuh</dt>
                                          <dd class="col-sm-8"><?php echo $data['sks_ditempuh'];?></dd>
                                          <dt class="col-sm-4">Lampiran file</dt>
                                          <dd class="col-sm-8"><?php if(empty($data['file_transkrip_nilai'])) { echo 'Tidak ada';} else { echo '<a class="btn btn-outline-primary btn-xs" title="Lihat/download" href="'.$data['file_transkrip_nilai'].'" target="_blank">Transkrip</a>';}?></dd>
                                          ...
                                          <dd class="col-sm-8"><?php if(empty($dt_jdwl['pengawas1']) && empty($dt_jdwl['pengawas2']) && empty($dt_jdwl['tgl_kompre']) && empty($dt_jdwl['jam_mulai']) && empty($dt_jdwl['jam_selesai']) && empty($dt_jdwl['ruang'])) { echo "Belum ada";} else { echo "Pengawas 1: $dt_p1[nm]
                                            <br/>Pengawas 2: $dt_p2[nm]
                                          ...
                                          <dd class="col-sm-8"><?php include "nilaiPesKompreUser.php";?></dd>
                                        </dl>
                                      </div>
                                    </div>
                                  </div>
    ...
    <script>
      $( '#modal-pre-cetak' ).on( 'show.bs.modal', function ( event ) {
          ...
          $.ajax( {
            type: "GET",
            url: "bodyPetunjukCetakPkompre.php",
            data: dataString,
            cache: false,
