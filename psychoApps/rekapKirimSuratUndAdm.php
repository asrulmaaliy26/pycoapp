    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
            ...
            <div class="row">
              ...
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                  <li class="breadcrumb-item active small">Surat Undangan</li>
                </ol>
              </div>
            </div>
          </div>
        ...
        <?php include 'pagination.php';
          ...
          $reload = "rekapKirimSuratUndAdm.php?pagination=true&keyword=$keyword";
          ...
          WHERE (ss.deskripsi LIKE '%$keyword%' OR ss.jenis_surat LIKE '%$keyword%' OR ss.file_surat LIKE '%$keyword%' OR ss.tgl_upload LIKE '%$keyword%' OR ss.bln_upload LIKE '%$keyword%' OR ss.thn_upload LIKE '%$keyword%' OR ss.catatan LIKE '%$keyword%' OR ss.executor LIKE '%$keyword%' OR ss.editor LIKE '%$keyword%' OR dp.nama LIKE '%$keyword%' OR ab1.nm_person LIKE '%$keyword%' OR ab2.nm_person LIKE '%$keyword%') AND ss.thn_upload='$tahun' AND ss.jenis_surat='3' GROUP BY ss.id ORDER BY ss.tgl_upload DESC";
          
          $result = mysqli_query($con, $sql);
          }else{
          $reload = "rekapKirimSuratUndAdm.php?pagination=true";
          $sql = "SELECT * FROM sending_surat WHERE thn_upload='$tahun' AND jenis_surat='3' ORDER BY tgl_upload DESC";
          $result = mysqli_query($con, $sql);
          }
          ...
          <div class="container-fluid">
            <div class="row">
              <div class="col-sm-4 mb-2">
                <form method="post" action="rekapKirimSuratUndAdm.php">
                  ...
                  <div class="input-group">
                    ...
                    <div class="input-group-append">
                      ...
                      <?php
                        if($_REQUEST['keyword']<>""){
                        ?>
                      <a class="btn btn-sm btn-warning" title="Kembali" href="rekapKirimSuratUndAdm.php"><i class="fas fa-sync"></i> Kembali</a>
                      <?php
                        }
                        ?>
                    </div>
              ...
              <div class="col-sm-8 mb-2">
                <a href="inputFileSuAdm.php?page=<?php echo $page;?>" class="btn btn-outline-danger btn-sm float-sm-right"><i class="fas fa-upload"></i> Upload file baru</a>
              </div>
            </div>
            <div class="row">
                      ...
                      <table class="table table-hover table-sm custom mb-0">
                        ...
                        <tbody>
                          ...
                          <tr data-widget="expandable-table" aria-expanded="false">
                            ...
                            <td class="text-center" width="12%"><a href="inputPenerimaFileSuAdm.php?id=<?php echo $data['id'];?>&page=<?php echo $page;?>" class="btn btn-outline-primary btn-xs btn-block" title="Input penerima file"><i class="fas fa-user-plus"></i> Input Penerima</a></td>
                            <td class="text-center" width="12%"><?php if(empty($data1['jumData'])) {echo '<a class="btn btn-outline-secondary btn-xs btn-block" onClick="return confirm(\'Belum ada penerima file\')" title="Belum ada penerima"><i class="fas fa-user-slash"></i> Hapus Penerima</a>';} else { echo '<a href="deletePenerimaFileSuAdm.php?id='.$data['id'].'&page='.$page.'" class="btn btn-outline-danger btn-xs btn-block" title="Hapus penerima file"><i class="fas fa-user-slash"></i> Hapus Penerima</a>';}?></td>
                            <td class="text-center pl-2" width="4%"><a href="editFileSuAdm.php?id=<?php echo $data['id'];?>&page=<?php echo $page;?>" role="button" class="btn btn-outline-warning btn-xs btn-block" title="Edit file"><i class="bi bi-pencil-square"></i></a></td>
                            <td class="text-center pr-2" width="4%"><a href="deleteFileSuAdm.php?id=<?php echo $data['id'];?>&page=<?php echo $page;?>" role="button" class="btn btn-outline-danger btn-xs btn-block" onClick="return confirm('Yakin data ini dihapus?')" title="Hapus file"><i class="bi bi-trash"></i></a></td>
                          </tr>
