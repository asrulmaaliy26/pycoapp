<?php
   ...
   $thn_upload_a = mysqli_real_escape_string($con,  $_POST[ 'thn_upload' ] );
   ...
   $nama_baru = "SK_".$id_berkas .uniqid().$date. '.' . end($temp);
   ...
   mysqli_query($con, "UPDATE sending_surat SET file_surat='$file_surat' WHERE id='$id' LIMIT 1");
    ...
    header("location:dataKfsSkPertahunAdm.php?page_a=$page_a&thn_upload=$thn_upload_a&page=$page&message=notifGagal");
    }
    ...
    header("location:dataKfsSkPertahunAdm.php?page_a=$page_a&thn_upload=$thn_upload_a&page=$page&message=notifEdit");
    exit;
    ?>