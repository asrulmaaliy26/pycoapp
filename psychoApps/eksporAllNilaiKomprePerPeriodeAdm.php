...
<html lang="en">
  <head>
    <title>Data Nilai Ujian Komprehensif <?php echo 'Tahap '.$dthp['tahap'].' ('.$djp['nm'].') '.$dsemester['nama'].' '.$dnta['ta'].'';?></title>
    <meta charset="utf-8">
    <style>
      table {
  ...
  <body>
    <?php
      ...
      header('Content-Disposition: attachment; filename=Data Nilai Ujian Komprehensif Tahap '.$dthp['tahap'].' ('.$djp['nm'].') '.$dsemester['nama'].' '.$dnta['ta'].'.xls');
      ?>
    <table style="border:none;">
      <thead>
        <tr style="border:none;">
          <th style="border:none;" colspan="12">Data Nilai Ujian Komprehensif <?php echo 'Tahap '.$dthp['tahap'].' ('.$djp['nm'].') '.$dsemester['nama'].' '.$dnta['ta'].'';?>
          <br>
          Passing Grade: <?php echo $didper['passing_grade'];?></th>
        </tr>
        <tr style="border:none;">
    ...
    <table>
      ...
      <tbody>
        <?php
          $no=0;
          $sql = "SELECT * FROM peserta_kompre INNER JOIN dt_mhssw ON peserta_kompre.nim=dt_mhssw.nim WHERE peserta_kompre.id_kompre='$id'  AND peserta_kompre.hasil_ujian!='' ORDER BY dt_mhssw.nama ASC";
        ...
        <tr>
          ...
          <td style="text-align:left;"> <?php if(empty($data['hasil_ujian'])) { echo "Belum ada";} elseif($data['hasil_ujian'] <= 100 && $data['hasil_ujian'] >= 85) { echo $data['hasil_ujian']. ' (A)';} elseif ($data['hasil_ujian'] <= 84 && $data['hasil_ujian'] >= 75) { echo $data['hasil_ujian']. ' (B+)';} elseif ($data['hasil_ujian'] <= 74  && $data['hasil_ujian'] >= 70) { echo $data['hasil_ujian']. ' (B)';} elseif ($data['hasil_ujian'] <= 69 && $data['hasil_ujian'] >= 65) { echo $data['hasil_ujian']. ' (C+)';} elseif ($data['hasil_ujian'] <= 64 && $data['hasil_ujian'] >= 60) { echo $data['hasil_ujian']. ' (C)';} elseif ($data['hasil_ujian'] < 60) {echo $data['hasil_ujian']. ' (D)';}?> <?php if($data['hasil_ujian'] <= 100 && $data['hasil_ujian'] >= $didper['passing_grade']) { echo "<span class='bg-primary'>Lulus</span>";} elseif($data['hasil_ujian'] < $didper['passing_grade'] && $data['hasil_ujian'] >= 1) { echo "<span class='bg-danger'>Gagal</span>";} elseif(empty($data['hasil_ujian'])) { echo "";}?> </td>
        </tr>
        <?php
          }
          ?>
