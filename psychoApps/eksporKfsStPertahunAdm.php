...
<html lang="en">
  ...
  <body>
    <?php
      ...
      header('Content-Disposition: attachment; filename=Data Kirim File Surat Tugas Tahun '.$thn_upload.'.xls');
      ?>
    <p><strong>Data Kirim File Surat Tugas Tahun <?php echo $thn_upload;?></strong></p>
    <table width="100%">
      <thead>
        <tr>
          <th width="4%">No</th>
      ...
      <tbody>
        <?php
          $no=0;
          $sql =  "SELECT * FROM sending_surat WHERE thn_upload='$thn_upload' AND jenis_surat='2' ORDER BY tgl_upload ASC";
          $result = mysqli_query($con, $sql);
          while($data = mysqli_fetch_array($result)) {
             $no++;
             $qry = "SELECT COUNT(*) AS jumData FROM sending_surat WHERE thn_upload='$thn_upload' AND jenis_surat='2'";
