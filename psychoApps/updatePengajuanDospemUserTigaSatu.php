...
$qry1 = "SELECT COUNT(id) AS jumData FROM pengelompokan_dospem_skripsi WHERE dospem_skripsi1='$_POST[dospem_skripsi1]' AND id_periode='$_POST[id_periode]'";
...
if($_POST['dospem_skripsi1'] != $dospem1 && $data1['jumData'] >= $d1['kuota1']) {
  header("location:editPengajuanDospemUserTigaSatu.php?id=$id&message=notifGagalDospem1");}

else if($_POST['dospem_skripsi1'] == $dospem2) {
  header("location:editPengajuanDospemUserTigaSatu.php?id=$id&message=notifGagalSama");}

else {

if($cek1 == 4 && $_POST['dospem_skripsi1'] != $dospem1) {
  $myqry1="UPDATE pengelompokan_dospem_skripsi SET dospem_skripsi1='$dospem_skripsi1',cek1='1',tgl_pengajuan='$tgl_pengajuan',thn_pengajuan='$thn_pengajuan' WHERE id='$id' LIMIT 1";
  mysqli_query($con, $myqry1) or die(mysqli_error($con));
  header("location:riwayatPengajuanDospemUser.php?message=notifEdit");}
  else {
  $myqry2="UPDATE pengelompokan_dospem_skripsi SET dospem_skripsi1='$dospem_skripsi1',tgl_pengajuan='$tgl_pengajuan',thn_pengajuan='$thn_pengajuan' WHERE id='$id' LIMIT 1";
