  ...
  <body class="hold-transition sidebar-mini layout-fixed">
            ...
            <div class="row mb-2">
              ...
              <div class="col-sm-8">
                <ol class="mt-2 breadcrumb float-sm-right">
                  <li class="breadcrumb-item active">Izin Magang Mandiri Individu</li>
                </ol>
              </div>
            </div>
          </div>
                ...
                <div class="card card-success card-outline card-outline-tabs">
                  <div class="card-header p-0 border-bottom-0">
                    <ul class="nav nav-tabs" role="tablist">
                      <li class="nav-item">
                        <a class="nav-link" href="formSimagiUser.php" role="tab" aria-selected="true">Form</a>
                      </li>
                      <li class="nav-item">
                        <a class="nav-link active" href="riwayatSimagiUser.php" role="tab" aria-selected="true">Riwayat</a>
                      </li>
                    </ul>
                  </div>
                      ...
                      <table class="table table-hover m-0 text-center table-sm custom">
                        <thead>
                          <tr>
                            ...
                            <th width="40%">Tempat magang</th>
                            <th width="12%">Status proposal</th>
                        ...
                        <tbody>
                          <?php
                            $no=0;
                            $sql = "SELECT * FROM magang WHERE nim=$dataku[nim] AND jenis_magang='1' ORDER BY id DESC";
                            $result = mysqli_query($con, $sql);
                            while ($data = mysqli_fetch_array($result)) {
                            
                            $qrykota = "SELECT * FROM dt_kota WHERE id='$data[kota_lts]'";
                            $reskota = mysqli_query($con,  $qrykota )or die( mysqli_error($con) );
                            $dtkota = mysqli_fetch_assoc( $reskota );
                            
                            $qryvalprop = "SELECT * FROM opsi_validasi WHERE id='$data[validasi_proposal]'";
                            $resvalprop = mysqli_query($con,  $qryvalprop )or die( mysqli_error($con) );
                            $dtvalprop = mysqli_fetch_assoc( $resvalprop );
                          ...
                          <tr>
                            ...
                            <td class="text-center"><?php echo $data['nama_obyek'].' di '.$dtkota['nm_kota'];?></td>
                            <td class="text-center"><?php echo $dtvalprop['nm'];?></td>
                            <td class="text-center pr-1"><?php echo $dtval['nm'];?></td>
                            <td width="4%" class="text-center pl-1"><?php if(empty($data['proposal'])) { echo "<a class='btn btn-outline-secondary btn-xs btn-block' onclick='return confirm(\"Tidak ada file.\")' title='Tidak ada file.' disabled><i class='fas fa-file-pdf'></i></a>";} else { echo "<a class='btn btn-outline-success btn-xs btn-block' title='Lihat/download file proposal' href=".$data['proposal']." target='_blank'><i class='fas fa-file-pdf'></i></a>";}?></td>
                            <td width="4%" class="text-center"><?php if($data['statusform']==1 OR $data['statusform']==4) { echo "<a class='btn btn-outline-warning btn-xs btn-block' onclick='return confirm(\"Yakin data ini diedit?\")' title='Yakin data ini diedit?' href='editSimagiUser.php?id=".$data['id']."'><i class='far fa-edit'></i></a>";} else { echo "<a class='btn btn-outline-secondary btn-xs btn-block' onclick='return confirm(\"Tidak bisa diedit. Permohonan surat telah diterima/diproses/selesai\")' title='Tidak bisa diedit. Permohonan surat telah diterima/diproses/selesai' disabled><i class='far fa-edit'></i></a>";}?></td>
                            <td width="4%" class="text-center"><?php if($data['statusform']==1 OR $data['statusform']==4) { echo "<a class='btn btn-outline-danger btn-xs btn-block' onclick='return confirm(\"Yakin data ini dihapus?\")' title='Yakin data ini dihapus?' href='deleteSimagiUser.php?id=".$data['id']."'><i class='far fa-trash-alt'></i></a>";} else { echo "<a class='btn btn-outline-secondary btn-xs btn-block' onclick='return confirm(\"Tidak bisa dihapus. Permohonan surat telah diterima/diproses/selesai\")' title='Tidak bisa dihapus. Permohonan surat telah diterima/diproses/selesai' disabled><i class='far fa-trash-alt'></i></a>";}?></td>
                            <td width="4%" class="text-center"><?php if(!empty($data['catatan'])) { echo "<a class='btn btn-outline-primary btn-xs btn-block' title='Lihat catatan' href='catatanSimagiUser.php?id=".$data['id']."'><i class='far fa-comment-dots'></i></a>";} else { echo "<a class='btn btn-outline-secondary btn-xs btn-block' onclick='return confirm(\"Tidak ada catatan.\")' title='Tidak ada catatan.' disabled><i class='far fa-comment-dots'></i></a>";}?></td>
    ...
    <script>
      $( '#modal-pre-cetak' ).on( 'show.bs.modal', function ( event ) {
          ...
          $.ajax( {
            type: "GET",
            url: "bodyPetunjukCetakSimagi.php",
            data: dataString,
            cache: false,
