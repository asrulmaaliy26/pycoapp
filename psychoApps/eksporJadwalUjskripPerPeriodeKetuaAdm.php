<?php include( "contentsConAdm.php" );
  ...
  $qidjdwl = "SELECT * FROM jadwal_ujskrip WHERE ketua_penguji='$id' AND id_ujskrip='$id_ujskrip'";
  $ridjdwl = mysqli_query($con, $qidjdwl)or die( mysqli_error($con));
  $didjdwl = mysqli_fetch_assoc($ridjdwl);

  $qpenguji = "SELECT * FROM dt_pegawai WHERE id='$didjdwl[ketua_penguji]'";
...
<html lang="en">
  ...
  <body>
    <?php
      ...
      $jdwl = "SELECT * FROM jadwal_ujskrip WHERE ketua_penguji='$id' AND id_ujskrip='$id_ujskrip' AND tgl_ujian='$tgl_ujian'";
      $rjdwl = mysqli_query($con,  $jdwl )or die( mysqli_error($con) );
      $djdwl = mysqli_fetch_assoc( $rjdwl );
         
      $qpenguji = "SELECT * FROM dt_pegawai WHERE id='$djdwl[ketua_penguji]'";
      $rqpenguji = mysqli_query($con, $qpenguji)or die( mysqli_error($con));
      $dqpenguji = mysqli_fetch_assoc($rqpenguji);
      ...
      include("extEksporJadwalUjskrip1Ketua.php");
      include("extEksporJadwalUjskrip2Ketua.php");
      include("extEksporJadwalUjskrip3Ketua.php");?>
