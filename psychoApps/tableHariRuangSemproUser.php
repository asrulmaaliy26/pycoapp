  ...
  <table class="table m-0 table-bordered table-sm small custom">
    <thead>
      <tr>
        ...
        <th width="64%"class="text-left"><?php if(empty($data['tgl_seminar']))  { echo "";} else { echo $dayList[$day].', '.bulanIndo($data['tgl_seminar']);}?></th>
      ...
      <tr>
        ...
        <th class="text-center pr-1">
          <?php if(empty($data['tgl_seminar']) && empty($data['ruang'])) { echo "<a type='button' class='btn btn-outline-secondary btn-flat btn-xs btn-block' title='Jadwal belum bisa dicetak' onclick='return confirm(\"Jadwal belum bisa dicetak\")'><i class='fas fa-print'></i> Jadwal Belum Bisa Dicetak</a>";} else { echo '<a type="button" href="cetakJadwalSemproPerPeriodeUser.php?id='.$data['id'].'" class="btn btn-success btn-flat btn-xs btn-block" title="Cetak jadwal" target="_blank"><i class="fas fa-print"></i> Cetak Jadwal</a>';}?>
        </th>
      </tr>
    </thead>
  </table>
