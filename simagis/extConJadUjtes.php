   ...
   <table class="table table-striped table-condensed table-bordered custom">
      <thead>
         <tr>
            ...
            <th class="text-center" width="18%">Judul Tesis</th>
      ...
      <tbody class="text-muted" style="font-size:13px;">
         <?php				           	  
            $no=0;
            $q =  "SELECT * FROM mag_jadwal_ujtes WHERE id_ujtes='$data[id_ujtes]' AND tgl_ujian='$data[tgl_ujian]' ORDER BY jam_mulai ASC";
            $r = mysqli_query($GLOBALS["___mysqli_ston"], $q);
            while($d = mysqli_fetch_array($r)) { 
            $no++;
            
            $query =  "SELECT * FROM mag_peserta_ujtes WHERE id='$d[id_pendaftaran]'";
