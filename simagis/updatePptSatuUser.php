<?php
  ...
  $qNip1 = "SELECT nip FROM mag_dospem_tesis WHERE id='$_POST[dospem_tesis1]'";
  ...
  $qry = "SELECT COUNT(dospem_tesis1) AS jumData FROM mag_pengelompokan_dospem_tesis WHERE dospem_tesis1='$_POST[dospem_tesis1]' AND id_periode='$_POST[id_periode]'";
  ...
  if($_POST['dospem_tesis1'] == $dospem2)
  {
  header("location:formPengajuanPt.php?message=notifGagalSama");
  }
  else if($data['jumData'] >= $d['kuota1']) 
  {
  header("location:formPengajuanPt.php?message=notifGagalDospem");
  }
  else
  {
  mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE mag_pengelompokan_dospem_tesis SET dospem_tesis1='$dospem_tesis1',nip_dospem_tesis1='$nip1',tgl_pengajuan='$tgl_pengajuan',thn_pengajuan='$thn_pengajuan' WHERE id='$id' LIMIT 1") or die(mysqli_error($GLOBALS["___mysqli_ston"]));    
