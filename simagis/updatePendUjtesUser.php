<?php
  ...
  $namafolder = "file_tesis/";
  	...
  	$nama_baru = $nim . '-' . $nmmhsw . '-' . $id_ujtes . '_tesis_'.$date. '.' . end($temp);
  ...
  $nf_kwitansi = "file_kwitansi_ujian_tesis/";
   ...
   $myqry="UPDATE mag_peserta_ujtes SET judul_tesis='$judul_tesis',tgl_pendaftaran='$tgl_pendaftaran',thn_pendaftaran='$thn_pendaftaran' WHERE id='$id' LIMIT 1";
  	mysqli_query($GLOBALS["___mysqli_ston"], $myqry) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
  	header("location:formPendUjTes.php?message=notifEdit");
  ?>